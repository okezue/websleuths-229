...
class EpisodeStore:
    def __init__(self,db_path:str|Path):
        ...
        self._db.execute("CREATE TABLE IF NOT EXISTS episodes("
            ...
            "body TEXT,ts TEXT,meta TEXT)")
        self._db.commit()
    def put(self,ep:Episode)->bool:
        try:
            self._db.execute(
                "INSERT INTO episodes(eid,url,title,body,ts,meta) VALUES(?,?,?,?,?,?)",
                (ep.eid,ep.url,ep.title,ep.body,ep.ts.isoformat(),json.dumps(ep.meta)))
            self._db.commit()
            return True
        except sqlite3.IntegrityError:
            return False
    def get(self,eid:str)->Episode|None:
        r=self._db.execute("SELECT url,title,body,ts,meta FROM episodes WHERE eid=?",(eid,)).fetchone()
        if not r:return None
        return Episode(url=r[0],title=r[1],body=r[2],
            ts=datetime.fromisoformat(r[3]),meta=json.loads(r[4]))
    ...
    def all(self)->list[Episode]:
        rows=self._db.execute("SELECT url,title,body,ts,meta FROM episodes").fetchall()
        return [Episode(url=r[0],title=r[1],body=r[2],
            ts=datetime.fromisoformat(r[3]),meta=json.loads(r[4])) for r in rows]
    def close(self):
        self._db.close()
