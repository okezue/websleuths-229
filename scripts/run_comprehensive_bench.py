...
def make_recipe_fn(name,tok,bs=2,lr=2e-4,ml=256,dn=4,dl=64):
    _last={"result":None}
    def fn(model,ds,dreams,steps=50,dbank=None):
...
def main():
    ap=argparse.ArgumentParser(description="Comprehensive CL benchmark with Claude KG extraction")
    ...
    pr(f"COMPREHENSIVE BENCHMARK: {args.model} recipe={args.recipe} lora_r={args.lora_r}")
    ...
    log.info("extraction: %s","Claude KG" if args.anthropic_key else "regex (no anthropic key)")
    log.info("exa_key: %s","set" if args.exa_key else "NOT SET")
    en=args.eval_n
    exn=args.example_n
    ...
    report={"config":{"model":args.model,"recipe":args.recipe,"lora_r":args.lora_r,
                       ...
                       "extraction":"claude_kg" if args.anthropic_key else "regex",
    ...
    log.info("Testing original model on ALL benchmarks — FULL datasets, no sampling")
    t1=time.time()
    ...
    pr("PHASE 2: SEQUENTIAL DOMAIN LEARNING WITH CLAUDE KG")
    log.info("Gates DISABLED — all topics will search + train")
    log.info("Extraction: %s","Claude KG" if args.anthropic_key else "regex fallback")
    extraction_backend="claude" if args.anthropic_key else "regex"
    cfg=WMCfg(
        search=SearchCfg(
            exa_api_key=args.exa_key,max_rounds=5,queries_per_round=6,
    ...
    for di,dom in enumerate(DOMAINS):
        ...
        dom_report={"topics":[],"backtest":{},"diags":[],"timing":{}}
        dom_dbank=None
        dom_ft_time=0.0
        dom_search_time=0.0
        dom_ft_steps=0
        ...
        for ti,topic in enumerate(topics):
            ...
            try:
                ...
                log.info("  [%s] pipe: action=%s claims=%s communities=%s asteps=%s dreams=%s",
                         ts(),action,pres.get("claims"),pres.get("communities"),
                         pres.get("adaptive_steps"),pres.get("dreams"))
            ...
            if tr:
                ...
                log.info("  [%s] TRAINED: loss=%.4f dream_loss=%.4f steps=%d lr=%.2e time=%.1fs",
                         ts(),tr.loss,tr.dream_loss or 0,tr.steps,tr.lr,t_topic)
    ...
    print(f"Extraction: {'Claude KG' if args.anthropic_key else 'regex'}")
    ...
    pr("COMPREHENSIVE BENCHMARK COMPLETE")
