...
gr=gate.check(eps)
print(f"gate: accept={gr.accept} sources={gr.n_sources} cons={gr.consistency:.3f}")
...
r1=EATRDRunner(lr=2e-4,max_steps=30,bs=2,temp=2.0,
    eps_min=0.01,alpha=0.5,rho=0.01,lam_init=1.0,
    max_len=256,dream_n=4,dream_len=128)
res1=r1.run(model,teacher,ds,P_DREAM,tok)
lam=res1.extras["lambda"]
ek=res1.extras["eps_k"]
print(f"  loss={res1.loss:.4f} dream={res1.dream_loss:.4f} steps={res1.steps}")
print(f"  lambda={lam:.4f} eps_k={ek:.4f}")
print("=== RECIPE 2: DPMU ===")
model2=copy.deepcopy(model)
teacher2=copy.deepcopy(teacher)
r2=DPMURunner(lr=2e-4,max_steps=30,bs=2,temp=2.0,
    n_dream_grads=1,max_len=256,dream_n=4,dream_len=128)
