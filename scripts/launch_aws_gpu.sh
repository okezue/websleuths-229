...
echo "PID=$!" > /tmp/wm_pid
echo "started at $(date)" >> /tmp/wm_run.log
STARTUP
)
...
INSTANCE_ID=$(aws ec2 run-instances \
    ...
    --user-data "$ENCODED" \
    --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=wm-agentic-scale}]" \
...
echo "Logs: ssh -i ~/.ssh/${KEY}.pem ubuntu@${PUBLIC_IP} 'tail -f /tmp/wm_run.log'"
echo "Stop: aws ec2 terminate-instances --instance-ids $INSTANCE_ID"
echo ""
echo "Results will be at /tmp/wm_agentic_scale_results.json on the instance"
echo "Copy: scp -i ~/.ssh/${KEY}.pem ubuntu@${PUBLIC_IP}:/tmp/wm_agentic_scale_results.json ."
