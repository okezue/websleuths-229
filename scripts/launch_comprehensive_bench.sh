...
HF="${HF_TOKEN:-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}"
ANTHROPIC="${ANTHROPIC_API_KEY:-}"

echo "=== Comprehensive CL Benchmark ==="
echo "Instance: $INST  AMI: $AMI  Disk: ${DISK_GB}GB"
echo "Recipe: $RECIPE  Model: $MODEL  eval_n=$EVAL_N"
echo "Extraction: $([ -n "$ANTHROPIC" ] && echo 'Claude KG' || echo 'regex')"
echo ""
...
pip install -q transformers peft datasets accelerate exa_py pydantic scipy tokenizers sentencepiece matplotlib anthropic
...
export ANTHROPIC_API_KEY="__ANTHROPIC__"
export TOKENIZERS_PARALLELISM=false
export PYTHONPATH=/home/ubuntu/repo:$PYTHONPATH

mkdir -p /tmp/wm_comprehensive_bench
...
nohup python3 scripts/run_comprehensive_bench.py \
    ...
    --recipe "__RECIPE__" \
    --lora-r 32 \
    --out /tmp/wm_comprehensive_bench \
    --lr 2e-4 \
    --bs __BS__ \
    --ml 256 \
    --min-steps 30 \
    ...
    --hf-token "$HF_TOKEN" \
    > /tmp/wm_comprehensive_bench/stdout.log 2>&1 &

echo "PID=$!" > /tmp/wm_pid
echo "started at $(date)" >> /tmp/wm_comprehensive_bench/bench.log
STARTUP
)
...
USERDATA=$(echo "$USERDATA" | sed "s|__ANTHROPIC__|$ANTHROPIC|g")
...
INSTANCE_ID=$(aws ec2 run-instances \
    ...
    --user-data "$ENCODED" \
    --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=wm-comprehensive-bench-${RECIPE}}]" \
...
echo "Logs:   ssh -i ~/.ssh/${KEY}.pem ubuntu@${PUBLIC_IP} 'tail -f /tmp/wm_comprehensive_bench/bench.log'"
echo "Stdout: ssh -i ~/.ssh/${KEY}.pem ubuntu@${PUBLIC_IP} 'tail -f /tmp/wm_comprehensive_bench/stdout.log'"
echo "Status: ssh -i ~/.ssh/${KEY}.pem ubuntu@${PUBLIC_IP} 'cat /tmp/wm_pid && ps aux | grep comprehensive'"
echo "Stop:   aws ec2 terminate-instances --instance-ids $INSTANCE_ID --region $REGION"
echo ""
echo "Copy results:"
echo "  scp -i ~/.ssh/${KEY}.pem -r ubuntu@${PUBLIC_IP}:/tmp/wm_comprehensive_bench/ ./comprehensive_bench_results/"
echo ""
echo "Monitor partial results:"
echo "  ssh -i ~/.ssh/${KEY}.pem ubuntu@${PUBLIC_IP} 'python3 -c \"import json;d=json.load(open(\\\"/tmp/wm_comprehensive_bench/results_partial.json\\\"));print(json.dumps({k:v for k,v in d.items() if k!=\\\"domains\\\"},indent=2))\"'"
